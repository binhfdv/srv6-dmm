...
public class Srv6Component {
    ...
    private void setUpMyUSidTable(DeviceId deviceId) {
        ...
        Ip6Address myUDX4 = getMyUDX4(deviceId);
        Ip6Address myUGTP4E = getMyUGTP4E(deviceId);
        Ip4Address src_addr = Ip4Address.valueOf("192.168.0.3");
        Ip4Address dst_addr = Ip4Address.valueOf("10.100.200.3");
        ...
        if (myUDX4 != null) {
            match = PiCriterion.builder()
                ...
                .build();
            action = PiAction.builder()
                .withId(PiActionId.of("IngressPipeImpl.srv6_end_dx4"))
                .build();
            myStationRule = Utils.buildFlowRule(
                 deviceId, appId, tableId, match, action);
            flowRuleService.applyFlowRules(myStationRule);
     ...
     * @return uDX for the device
     */
    private Ip6Address getMyUDX4(DeviceId deviceId) {
        return getDeviceConfig(deviceId)
                .map(Srv6DeviceConfig::myUDX4)
                .orElse(null);
    }
     ...
     * @return uDX for the device
     */
    private Ip6Address getMyUGTP4E(DeviceId deviceId) {
        return getDeviceConfig(deviceId)
                .map(Srv6DeviceConfig::myUGTP4E)
                .orElse(null);
    }
}